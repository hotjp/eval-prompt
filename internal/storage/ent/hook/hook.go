...
// The ModelAdaptationFunc type is an adapter to allow the use of ordinary
// function as ModelAdaptation mutator.
type ModelAdaptationFunc func(context.Context, *ent.ModelAdaptationMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ModelAdaptationFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ModelAdaptationMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ModelAdaptationMutation", m)
}
