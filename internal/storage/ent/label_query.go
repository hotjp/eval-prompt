...
func (_q *LabelQuery) QueryAsset() *AssetQuery {
...
func (_q *LabelQuery) WithAsset(opts ...func(*AssetQuery)) *LabelQuery {
	query := (&AssetClient{config: _q.config}).Query()
...
func (_q *LabelQuery) loadAsset(ctx context.Context, query *AssetQuery, nodes []*Label, init func(*Label), assign func(*Label, *Asset)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Label)
	for i := range nodes {
		if nodes[i].asset_labels == nil {
			continue
		}
		fk := *nodes[i].asset_labels
