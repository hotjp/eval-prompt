...
import { Card, Tag, Button, Space, Spin, message, Row, Col, Statistic, Table, Collapse } from 'antd'
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Card>
        <Row gutter={16}>
          <Col span={6}>
            <Statistic title="Overall Score" value={report!.overall_score} precision={2} suffix="/ 1.0" />
          </Col>
          <Col span={6}>
            <Statistic title="Deterministic Score" value={report!.deterministic_score} precision={2} suffix="/ 1.0" />
          </Col>
          <Col span={6}>
            <Statistic title="Rubric Score" value={report!.rubric_score} precision={2} suffix="/ 1.0" />
          </Col>
          <Col span={6}>
                    ...
                    <div style={{ marginTop: 4 }}>
                      {calls.slice(0, 3).map((call) => (
                        <Tag
                          ...
                          onClick={() => navigate(`/executions/${call.id}/calls`)}
                        >
                          {call.run_id.slice(-8)}
                        </Tag>
                      ))}
      ...
      <Card title="LLM Calls">
        <Table
          ...
          columns={[
            { title: 'ID', dataIndex: 'id', key: 'id', render: (id: string) => id.slice(-8) },
            {
              title: 'Status',
              dataIndex: 'status',
              key: 'status',
            ...
            },
            {
              title: 'Action',
              key: 'action',
              render: (_: unknown, record: LLMCall) => (
                <Button size="small" onClick={() => navigate(`/executions/${record.id}/calls`)}>
                  View
                </Button>
              ),
            },
